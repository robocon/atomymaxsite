        ...
        <TR>
		  ...
		  &nbsp;&nbsp;<IMG SRC="images/menu/textmenu_admin.gif" BORDER="0"><BR>
				...
				<TR>
					...
					<BR><BR>
<A HREF="?name=admin&file=user"><IMG SRC="images/admin/admins.gif"  BORDER="0" align="absmiddle"> <?=_ADMIN_USER_MENU_LIST;?></A> &nbsp;&nbsp;&nbsp;<A HREF="?name=admin&file=user&op=admin_add"><IMG SRC="images/admin/user.gif"  BORDER="0" align="absmiddle"> <?=_ADMIN_USER_MENU_ADD_NEW;?></A> &nbsp;&nbsp;&nbsp;<A HREF="?name=admin&file=groups"><IMG SRC="images/admin/keys.gif"  BORDER="0" align="absmiddle"> <?=_ADMIN_USER_MENU_LEVEL;?></A> &nbsp;&nbsp;&nbsp;<A HREF="?name=admin&file=groups&op=group_add"><IMG SRC="images/admin/share.gif"  BORDER="0" align="absmiddle"> <?=_ADMIN_USER_MENU_ADD_LEVEL;?></A>&nbsp;&nbsp;&nbsp;<A HREF="?name=admin&file=user&op=detail_log"><IMG SRC="images/admin/7_40.gif"  BORDER="0" align="absmiddle"> <?=_ADMIN_LOG_MENU_LINK;?></A>
<BR><BR>
...
//	echo"<meta http-equiv=\"refresh\" content=\"3;URL=?name=member\" />";
...
else if($op == "detail_log"){

	$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
	$limit = 20 ;
	$SUMPAGE= $db->num_rows(TB_ADMINLOG,"id","");
	...
	$goto = ($page-1)*$limit ;
?>
 <form action="?name=admin&file=user&op=log_del&action=multidel" name="myform" method="post">
 <table width="100%" cellspacing="0" cellpadding="0" class="grids">
  <tr  class="odd">
...
$res['log'] = $db->select_query("SELECT * FROM ".TB_ADMINLOG." ORDER BY id  DESC LIMIT $goto, $limit ");
...
?>
    <tr <?php echo $ColorFill; ?> >
     <td width="44">
      ...
      <a href="javascript:Confirm('?name=admin&file=user&op=log_del&page=<? echo $page;?>&id=<? echo $arr['log']['id'];?>&prefix=<? echo $arr['log']['post_date'];?>','<?=_ADMIN_BUTTON_DEL_MESSAGE;?>');"><img src="images/admin/trash.gif"  border="0" alt="<?=_ADMIN_BUTTON_DEL;?>" ></a>
     </td> 
     <td align="center" width="50"><a href="?name=admin&file=user&op=log_map&id=<? echo $arr['log']['id'];?>"><? echo $arr['log']['visitor_ip'];?></a></td>
...
<?
	...
	//////////////////////////////////////////// กรณีลบ Multi
		...
		while(list($key, $value) = each ($_POST['list'])){
			$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
			$res['visitor'] = $db->select_query("SELECT * FROM ".TB_ADMINLOG." WHERE id='".$value."' ");
			$arr['visitor'] = $db->fetch($res['visitor']);
			$db->del(TB_ADMINLOG," id='".$value."' "); 
			$db->closedb ();
		...
		$ProcessOutput .= "<A HREF=\"?name=admin&file=user&op=detail_log\"><B>"._ADMIN_LOG_MESSAGE_GOBACK."</B></A>";
		$ProcessOutput .= "<meta http-equiv='refresh' content='1; url=?name=admin&file=user&op=detail_log&page=".$_GET['page']."'>";
		$ProcessOutput .= "</CENTER>";
		$ProcessOutput .= "<BR><BR>";
	...
	//////////////////////////////////////////// กรณีลบ Form
			...
			$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
			$res['visitor'] = $db->select_query("SELECT * FROM ".TB_ADMINLOG." WHERE id='".$_GET['id']."' ");
			$arr['visitor'] = $db->fetch($res['visitor']);

		$db->del(TB_ADMINLOG," id='".$_GET['id']."' "); 
		$db->closedb ();
...
$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
$query = $db->select_query("SELECT * FROM ".TB_ADMINLOG." where id='".$_GET['id']."' ");
$row = $db->fetch($query);
$lat = $row['latitude'];
$long = $row['longitude'];
$city = $row['city'];
...
?>
    ...
    <script src="http://maps.google.com/maps?file=api&amp;v=2&amp;key=<?php echo _GOOGLE_SEND_API_KEY;?>" type="text/javascript"></script>  
