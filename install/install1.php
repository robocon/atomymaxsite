...
$language_install = trim( mosGetParam( $_POST, 'language_install', 'thai_utf-8' ) );
...
$install_iso = trim( mosGetParam( $_POST, 'install_iso', 'thai_utf-8' ) );
...
include( 'language/install_thai_utf-8.php' );
...
function check() {
	...
	if ( f.DBhostname.value == '' ) {
		...
		f.DBpassword.focus();
		formValid=false;
	} else if ( f.DBverifypassword.value != f.DBpassword.value ) {
		alert('<?php echo 'Password database not match'; ?>');
		f.DBverifypassword.focus();
		formValid=false;
...
<div id="wrapper">
	<div id="header">
		<div id="atomymaxsite">
    		<?php echo '<font color="#FF9900"><strong>Version: ' ._SCRIPT.'</strong></font>'; ?>
    	</div>
	</div>
</div>
